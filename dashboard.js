...
ProgressReport.View = {
    createProgressBar: async () => {
        ProgressReport.Records = await ProgressReport.Controller.getProgressBarData();
        const myRecordset = new ECP.EC_Recordset(ProgressReport.Records);
        let progress = 0;
        progress = myRecordset.Item("Completed");
        const progressBar = `
            <div class="progress-bar">
                <span class="progress" style="width: ${progress};"></span>
            </div>`;
        document.getElementById("percentage-value").textContent = progress;
...
async function getCurrentUserInfo(manager1) {
    ...
    usersTableView.AddColumn("Users_Authentications^Authentications.AuthenticationID");
...
async function getUsers() {
    ...
    while (!currentUserInfoRecordSet.EOF) {
        ...
        const authId = currentUserInfoRecordSet.Item("Users_Authentications^Authentications.AuthenticationID");
        const username = currentUserInfoRecordSet.Item("UserName");
        const img = currentUserInfoRecordSet.Item("DocumentID");

        createUserColumn(id, authId, username, img);
    ...
    paylinkTeamUsers.AddColumn("Users_Authentications^Authentications.AuthenticationID");
    ...
    paylinkTeamUsers.AddFilter("Users_Managers1^Managers1.Name", teamLead, ECP.EC_Operator.Equals);
    ...
    while (!usersRecordSet.EOF) {
        ...
        const authId = usersRecordSet.Item("Users_Authentications^Authentications.AuthenticationID");
        const username = usersRecordSet.Item("UserName");
        const img = usersRecordSet.Item("DocumentID");
...
async function getTasks(id, username) {
    ...
    today = `${mm}/${dd}/${yyyy}`;
    salesExeTasks.AddFilter("TimeUpdated", EC_Fmt.DateParseFormat(today), ECP.EC_Operator.LessThanEquals);
    ...
    while (!taskRecordSet.EOF) {
        ...
        // createtaskList(id, taskID, taskTitle, shortname, customerID, username, task, objType, unread, priority, daysOld);
        taskRecordSet.MoveNext();
    }
    const sortedTasks = await sortTaskList(taskArray);
    for (let sortIndex = 0; sortIndex < sortedTasks.length; sortIndex++) {
        await createtaskList(sortedTasks[sortIndex].ID,
            sortedTasks[sortIndex].TaskID,
            sortedTasks[sortIndex].TaskTitle,
            sortedTasks[sortIndex].ShortName,
            sortedTasks[sortIndex].CustomerID,
            sortedTasks[sortIndex].Username,
            sortedTasks[sortIndex].Task,
            sortedTasks[sortIndex].ObjType,
            sortedTasks[sortIndex].Unread,
            sortedTasks[sortIndex].Priority,
            sortedTasks[sortIndex].DaysOld);
    }
}

async function sortTaskList(taskArray) {
    // const newSort = taskArray.sort(taskArray.DaysOld);
    const newSort = taskArray.sort((firstTask, nextTask) => nextTask.Priority - firstTask.Priority || firstTask.DaysOld - nextTask.DaysOld);
    return newSort;
}
...
function createUserColumn(id, authId, name, img) {
    const userCol = `<div class="parent" id="${id}">
                        ...
                        <div class="parentCard" userId="${id}" id="${name}" auth-id="${authId}"></div>
                    </div>`;
...
async function createtaskList(id, taskID, taskTitle, shortname, customerID, username, task, objType, unread, priority, daysOld) {
    // const ReportData = await ProgressReport.Controller.getContributorsData();
    const checkKanbanData = await getStoryPoints(taskID);
    let taskData = "";
    taskData += `<div class="child bg-white text-dark ${username} ${priority === "1" ? "Priority" : ""} default d-flex flex-column" task-id="${taskID}" assigned-to="${id}" customer-id="${customerID}" draggable="true">
                    <div class="taskData">
    ...
    if (objType === "1") {
        ...
        taskData += '               <span class="badge badge-success">Feature</span>';
    }
                        ...
                        <div style="display:flex;margin-left:auto;width:40px;justify-content:space-between;">
                            <div class="kanban-container">
                                <a style="color:#000" target="_blank" href="https://encompasstech.com/Home?DashboardID=187332&Tab=You&TaskID=${taskID}&View=Task&UserID=${UserID}">
                                    ...
                                    ${checkKanbanData.length > 0 ? `<span class="notif"></span>` : ""}
                                </a>
                            </div>
...
function createTaskDetails(taskDetailsArr) {
    ...
    for (let i = 0; i < taskDetailsArr.length; i++) {
        ...
        if (taskDetailsArr[i].TaskDetails[3].startsWith("<br>") || taskDetailsArr[i].TaskDetails[3].startsWith("</br>")) {
            taskDetails = taskDetailsArr[i].TaskDetails[3].replace("<br>", "");
        } else {
            taskDetails = taskDetailsArr[i].TaskDetails[3];
        }
        if (!taskDetails.includes("Assigned to") && !taskDetails.includes("Status changed")) {
            const collapse = `<div class="d-flex align-items-start taskDetailsSection">
                                    <a class="ews-icon-edit editThisTaskDetail hidden" task-id='${taskDetailsArr[i].TaskID}' task-detail-id="${taskDetailsArr[i].TaskDetailID}" task="${taskDetailsArr[i].Task}"></a>
                                    <p class="px-2 py-2 taskDetailId" id="${taskDetailsArr[i].TaskDetailID}">
                                        ${EC_Fmt.ReplaceAll(EC_Fmt.ReplaceAll(taskDetails, "<hr>", ""), "<hr />", "")}
                                    </p>
                                </div><hr>`;
            document.getElementById(`taskDetails${taskDetailsArr[i].TaskID}`).innerHTML += collapse;
        }
    }
...
function createStoryPoints(storyArr, func, taskID) {
    ...
    const icons = [
        "ews-icon-assign",
        "ews-icon-chainbroken",
        "ews-icon-computer",
        "ews-icon-test",
        "ews-icon-circleo"
    ];

    const focusColor = "#626262";
    ...
    for (let i = 0; i < storyArr.length; i++) {
        ...
        if (Status === "Assigned") {
            counts[i] = StoryPoints;
            if (counts[i] > 0) {
                colors[i] = focusColor;
            }
        } else if (Status === "Defect Found") {
            counts[i] = StoryPoints;
            if (counts[i] > 0) {
                colors[i] = focusColor;
            }
        ...
        } else if (Status === "Confirmation Needed") {
            counts[i] = StoryPoints;
            if (counts[i] > 0) {
                colors[i] = focusColor;
            }
    ...
    printStoryPoints(counts, icons, colors, taskID, func, statuses);
}
function printStoryPoints(counts, icons, colors, taskID, func, statuses) {
    let html = `<div class="story-status-container-${taskID}">
                 <div style="display:flex;flex-wrap: wrap;flex-direction:row;border: none;align-items:center;justify-content:center;">`;
    for (let i = 0; i < statuses.length; i++) {
        html += `
        <div class="flip-card">
            <div class="flip-card-inner">
                <div class="flip-card-front" style="background:${colors[i]}">
                    <div class="story-count">
                        ${counts[i]}
                    </div>
                    <div class="story-status">
                        <i class="${icons[i]}" style="font-size:12px;"></i>
                    </div>  
                </div>
                <div class="flip-card-back" style="background:${colors[i]}">
                    <p>${statuses[i]}</p>
                </div>
            </div>
        </div>`;
...
async function toggle(event) {
    const taskID = event.currentTarget.parentElement.parentElement.parentElement.getAttribute("task-id");
    const status = event.currentTarget.parentElement.parentElement.parentElement.getAttribute("assigned-to");
    const storyContainer = content => event.target.closest(".default").querySelector(".donutSpinner").insertAdjacentElement("afterend", content);
    const taskDetails = document.getElementById(`taskDetails${taskID}`);
    const storyPointContainer = document.querySelector(`.story-status-container-${taskID}`);
    const taskDetailsSection = taskDetails.querySelectorAll(".taskDetailsSection");
    ...
    const elements = event.target.closest(".default").querySelectorAll(".taskDetails");
...
function showUtilizationModal(event) {
    const taskID = event.target.closest(".default") ? event.target.closest(".default").getAttribute("task-id") : "";
    ECP.Dialog.ShowDialog("Add Utilization Hours", utilizationForm(taskID), {
        width: "300",
        cancelButtonTitle: "Cancel",
        confirmButtonTitle: "Confirm",
        cancelButtonAction: () => { /* cancel code here */ },
        confirmButtonAction: async (modal) => {
            modal.style.display = "block";
            const form = modal.querySelector("#UtilizationForm");
            const taskIDInput = document.querySelector("ecp-auto-complete[inputname='TaskID']");
            const formData = new FormData(form);
            if (taskID) {
                formData.append("TaskID", taskID);
            } else if (taskIDInput) {
                if (taskIDInput.shadowRoot.querySelector("#TaskIDHidden")) {
                    formData.append("TaskID", taskIDInput.shadowRoot.querySelector("#TaskIDHidden").value);
                }
            }
...
function verifyInputFields(form) {
    ...
    if (!EC_Fmt.isNull(errors)) {
        ECP.HTML.Snackbar(`${errors.length > 1 ? errors.join(" and ") : errors[0]} ${errors.length > 1 ? "are" : "is a"} required field${errors.length > 1 ? "s" : ""}. ${errors.length > 1 ? "They" : "It"} must contain a valid value.`, "Error");
    }
    return valid;
}
...
function blurTime() {
    ...
    const timeStartSeconds = (parseInt(timeStartArray[0], 10) * 60 * 60) + (parseInt(timeStartArray[1], 10) * 60);
    const timeEndedSeconds = (parseInt(timeEndedArray[0], 10) * 60 * 60) + (parseInt(timeEndedArray[1], 10) * 60);
...
function utilizationForm(taskID) {
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
        ...
        <span style="width:180px">
            ${taskID || `<ecp-auto-complete inputName="TaskID" refTable="Tasks" refField="TaskID" refFieldDisplay="TaskID" dataType="ECP.DataType._Integer" refFieldDisplayDataType="ECP.DataType._Text" isSearch="false"></ecp-auto-complete>`}
            </span>
    </div>`);
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
        ...
        <span style="width:180px;"><ecp-input type="textarea" id="Memo" name="Memo" style="width:180px;"></ecp-input></span>
    </div>`);
...
function ptoForm() {
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
        ...
        <span style="width:180px;"> <ecp-input style="box-sizing: border-box;"type="text" valuesArr="${_Users.map(({ ID, Username }) => `${ID},${Username}`).join(";")}" ></ecp-input></span>
    </div>`);
...
function dragEnd(e) {
    ...
    for (let i = 0; i < element.length; i++) {
        ...
        if (movedChild !== newUser) {
            ...
            element[i].setAttribute("class", `child bg-white selectedChild text-dark ${newCSSClass} d-flex flex-column`);
            ...
            myTableEdit.SaveRecord().then(() => {
                document.getElementById("toastSave").classList.remove("show", "bg-save");
                document.getElementById("toast").classList.add("show", "bg-success");
                if (element.length > 1) {
                    document.getElementById("toast").textContent = `Successfully Updated ${element.length} Tasks`;
                } else {
                    document.getElementById("toast").textContent = `Successfully Updated Task ${movedChildTaskId}`;
                }
                setTimeout(() => {
                    document.getElementById("toast").classList.remove("show", "bg-success");
                }, 3000);
            });
            // .catch(() => {
...
function drop(event) {
    ...
    for (let i = 0; i < elements.length; i++) {
        document.getElementById(`${event.currentTarget.getAttribute("id")}`).appendChild(elements[i]);
    }
}
...
function appendPtoMenu() {
    if (UserID === 21522) {
        if (!document.getElementById("pto")) {
            ...
            newElement.addEventListener("click", () => { showPtoModal(); setDate(); });
...
async function main() {
    ...
    await ProgressReport.View.createProgressBar();
    ...
    document.getElementById("dropdown-button").addEventListener("click", appendPtoMenu);
    document.getElementById("util").addEventListener("click", showUtilizationModal);
    document.getElementById("pm-task-button").innerHTML = ECP.HTML.Button("pm-taks-button", "PM Tasks", ECP.HTML.Icons.Lists, ECP.HTML.ButtonActionType.Link, "", "ActionButton ActionButtonCustom");
    document.getElementById("pm-task-button").href = "Home?DashboardID=100100&TableName=Tasks&SelectDisplayInParent=TaskID%2CUnread%2CCustomerID%2CTask%2CAssignToUserID%2CObjectiveTypeID%2CObjectiveID%2CTaskStatusID%2CCustomField505%2CCustomField501%2CCustomField502%2CCustomField506%2CTimeUpdated%2CTasks_ZZ_Features%5EZZ_Features_Users%5EUsers.UserName&SelectMaxRecords=5000&SelectSort=CustomField506&Parameters=F:AssignToUserID~V:16446~O:E|F:TaskStatusID~V:1^2^3^4^5^8^23^25^27^28^29^31^32^35^40^44^45^46^47^66^67^68^69^70^71^72^73^76^77^78^79^82^83^84^85^86^88^89^90^92^93^95^100^101^102^106~O:E|F:TimeUpdated~V:10%2F24%2F2018%205%3A31%3A26%20PM~O:GE|F:Tasks_ZZ_Features^ZZ_Features_Users^Users.UserName~V:Sayco^YZ^Wilfred^Jericho^Erwin~O:E";
    document.getElementById("kpi-button").innerHTML = ECP.HTML.Button("kpi-button", "KPI", ECP.HTML.Icons.FavoriteOn, ECP.HTML.ButtonActionType.JSEvent, "", "ActionButton ActionButtonCustom");
    document.getElementById("kanban-button").innerHTML = ECP.HTML.Button("kanban-button", "Kanban Board", ECP.HTML.Icons.EMScheduleLight, ECP.HTML.ButtonActionType.Link, "", "ActionButton ActionButtonCustom");
